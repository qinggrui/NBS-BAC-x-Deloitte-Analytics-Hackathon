...
setwd("C:/Users/Qing Rui/Desktop/BAC_x_Deloitte_Business_Analytics_Hackathon/NBS BAC Business Analytics Hackathon - Team FinHack/team FinHack_Datasets used")
...
a3=head(annom3)
view(a)
write_xlsx(a3,"C:/Users/Qing Rui/Desktop/BAC_x_Deloitte_Business_Analytics_Hackathon/NBS BAC Business Analytics Hackathon - Team FinHack/team FinHack_Datasets used/a3.xlsx")
ggplot(data=a3, aes(x=Vendor, y= Duplicated_Transaction_Sum, group = Vendor, fill= Vendor)) + 
  geom_bar(stat = "identity", position = position_dodge(), width =  0.5)+geom_text(aes(label = signif(Duplicated_Transaction_Sum)), nudge_y = 3)

ggplot(data=a3, aes(x=Vendor, y= Perc_overpaid, group = Vendor, fill= Vendor)) + 
  geom_bar(stat = "identity", position = position_dodge(), width =  0.5)+geom_text(aes(label = signif(Perc_overpaid)), nudge_y = 3)
...
b1= aggregate(s3.1$`Total Amount`, by=list(AccProv=s3.1$`Bank Account`), FUN=sum) 
b1= b1[order(-b1$x),] #sort transaction according from highest to lowest
sum(b1$x) # sum of unique transaction values is $33,226,587 (Total flow transactions)
count(b1) # 3 unique vendors/accounts


b2=subset(s3,duplicated(s3))
view(b2)
count(b2)
b2.1= aggregate(b2$`Total Amount`, by=list(AccProv=b2$`Bank Account`), FUN=sum) 
b2.1= b2.1[order(-b2.1$x),]
sum(b2.1$x) # sum of unique transaction values is $19,903,890 (Total flow transactions)
count(b2.1) # 3 unique vendors/accounts

#merge annomalies and duplicated flow transaction sum
ab2=merge(b1, b2.1, by=("AccProv"), all=TRUE) #merge duplicated flow transaction with unique flow transaction to see which vendors have unique transactions and which dont have
ab2=ab2[order(-ab2$`x.y`),]
colnames(ab2)=c('Vendor Bank Account', 'Unique_Paid_Sum', 'Duplicated_Paid_Sum')
ab2$Perc_overpaid=((ab2$`Duplicated_Paid_Sum`/ab2$Unique_Paid_Sum)*100) #obtain percentage overpaid of data points/vendor

view(ab2)

write_xlsx(ab2,"C:/Users/Qing Rui/Desktop/BAC_x_Deloitte_Business_Analytics_Hackathon/NBS BAC Business Analytics Hackathon - Team FinHack/team FinHack_Datasets used/ab2.xlsx")
ggplot(data=ab2, aes(x=`Vendor Bank Account`, y= Duplicated_Paid_Sum, group = `Vendor Bank Account`, fill= `Vendor Bank Account`)) + 
  geom_bar(stat = "identity", position = position_dodge(), width =  0.5)+geom_text(aes(label = signif(Duplicated_Paid_Sum)), nudge_y = 3)

ggplot(data=ab2, aes(x=`Vendor Bank Account`, y= Perc_overpaid, group =`Vendor Bank Account`, fill=`Vendor Bank Account`)) + 
  geom_bar(stat = "identity", position = position_dodge(), width =  0.5)+geom_text(aes(label = signif(Perc_overpaid)), nudge_y = 3)
...
#================ Tax Miscalculation Fraud ========
transaction = data.table(read_excel("Credit Card Data.xlsx", sheet="Transaction Data", na = c("-","NA","NULL","")))
leave = data.table(read_excel("Credit Card Data.xlsx", sheet="Leave", na = c("-","NA","NULL","")))
names(transaction) <- gsub(" ", "_", names(transaction))
names(leave) <- gsub(" ", "_", names(leave))

unique(transaction[,Vendor_Location]) 
#all in Australia so can assume 10% GST
x = transaction[is.na(Vendor_Location),]


subtran = transaction[Tax_Code == 'P1',. (Employee_Number, Comment, Expense_Date, Tax_Code, Expense_Amount, Net_Amount, Tax_Amount, Transaction_ID)]
subtran[,cortax := (Net_Amount*0.1)-Tax_Amount][, corfin := (Net_Amount*1.1)-Expense_Amount]

notrounding = subtran[cortax > 1|cortax < -1,]

#NOT FOUND
# ========== Tax on Tax Exempted Vendors ==========

p2tax = unique(transaction[Tax_Code == 'P2', Vendor_Name])
canttax = transaction[Tax_Code == "P1",][Vendor_Name %in% p2tax,]
...
ghostbank = setorder(master[Bank_Acct %in% master[duplicated(master$Bank_Acct),Bank_Acct],][, .(Employee_Master, Job_Desc, Mobile_Phone, Bank_Acct)],Bank_Acct)
...
netnotzerotot = erratictot[, .(NetChange= sum(change, na.rm=TRUE)), by = Employee_ID][NetChange!= 0,]
...
#Relevant ONLY
ppersonbas = setorder(payslips[Relevant_Income == "Yes",.(MonthlyAmt =sum(Amount,na.rm=TRUE)),by = .(Employee_ID,Date)], Employee_ID,Date)
...
netnotzerobas = setorder(erraticbas[, .(NetChange= sum(change, na.rm=TRUE)), by = Employee_ID][NetChange!= 0,], -NetChange)
...
empven = master[Bank_Acct %in% empven, .(Employee_Master, Bank_Acct)]
